...
export default function JobsPage() {
  ...
  const applyInventoryChange = async ({
    ...
    if (rpcErr) {
      throw new Error(
        rpcErr.message.includes("chk_balance_quantity")
  ...
  const save = async () => {
    ...
    if (
      ...
      setError(
        "Job No., Client, Location, and at least one line item are required."
      );
      return;
    }

    const invalidLine = d.lineItems.some(
      (li) => !li.part_id || Number(li.qty) <= 0
    );

    if (invalidLine) {
      setError(
        "Every part line must have a selected part and quantity greater than zero."
      );
      return;
    }
    ...
    try {
      ...
      if (modal.mode === "create") {
        const { data: jobRow, error: jobErr } =
          await supabase
            .from("jobs")
            .insert({
              org_id: orgId,
        ...
        // Deduct stock at the selected location.
        ...
        await logActivity(
          `Logged job ${d.job_no} for ${d.client} (from ${
            locationById(d.location_id)?.name || ""
          })`
        );
      } else {
        ...
        // Restore the original job quantities.
        ...
        // Deduct the new quantities at the selected location.
        for (const li of d.lineItems) {
          await applyInventoryChange({
            partId: li.part_id,
            locationId: d.location_id,
      ...
      setError(
        e.message || "Something went wrong saving the job."
      );
    } finally {
      setSaving(false);
    }
  ...
  const remove = async () => {
    ...
    try {
      // Return all parts used by the job to stock.
      ...
      setError(
        e.message || "Something went wrong deleting the job."
      );
    }
  ...
  const filtered = jobs.filter((j) =>
    `${j.job_no} ${j.client} ${j.address || ""}`
      .toLowerCase()
      .includes(q.toLowerCase())
  );

  const lineTotals = (lineItems) => ({
    qty: lineItems.reduce(
      (s, li) => s + Number(li.qty || 0),
      0
    ),
    cost: lineItems.reduce(
      (s, li) =>
        s +
        Number(li.qty || 0) *
  ...
  return (
    <Nav title="Jobs">
      <div className="p-4 md:p-6">
        ...
        {parts.length === 0 && (
          <div className="text-sm text-amber-400 mb-3">
            Add at least one part in the Parts Catalog before
            logging a job.
          </div>
        )}
                ...
                <tbody>
                  ...
                  {filtered.length === 0 && (
                    <tr>
                      ...
                      >
                        No jobs yet — log your first one above.
                      </Td>
                    </tr>
                  )}
                </tbody>
      ...
      {confirmDelete && (
        <ConfirmModal
          title="Delete Job"
          message={`Delete "${confirmDelete.job_no}"? Parts used on this job will be returned to stock at ${
            locationById(confirmDelete.location_id)
              ?.name || "its original location"
...
function JobModal({
  ...
  const removeLine = (i) =>
    updateField(
      "lineItems",
      d.lineItems.filter((_, idx) => idx !== i)
    );

  const totalQty = d.lineItems.reduce(
    (s, li) => s + Number(li.qty || 0),
    0
  );

  const totalCost = d.lineItems.reduce(
    (s, li) =>
      s +
      Number(li.qty || 0) *
        Number(li.part_cost || 0),
  ...
  const totalSale = d.lineItems.reduce(
    (s, li) =>
      s +
      Number(li.qty || 0) *
        Number(li.sale_cost || 0),
  ...
  return (
    <ModalShell
      title={`${modal.mode === "create" ? "Log" : "Edit"} Job`}
      icon={Briefcase}
      onClose={onCancel}
      wide
    >
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4">
        <Field label="Job No. / Invoice No.">
          <input
            ...
            value={d.job_no}
            onChange={(e) =>
              updateField("job_no", e.target.value)
            }
          />
        </Field>
        ...
        <Field label="Date">
          <input
            ...
            value={d.job_date}
            onChange={(e) =>
              updateField("job_date", e.target.value)
            }
          />
        </Field>
        ...
        <Field label="Technician">
          <input
            ...
            value={d.technician}
            onChange={(e) =>
              updateField("technician", e.target.value)
            }
          />
        </Field>
      </div>
      ...
      <div className="mt-3 border border-slate-800 rounded">
        ...
        {d.lineItems.map((li, i) => (
          <div
            key={i}
            className="grid grid-cols-[2fr_0.7fr_1fr_1fr_auto] gap-2 px-3 py-2 items-center border-b border-slate-800/60 last:border-0"
          >
            <PartPicker
              orgId={orgId}
              value={li.part_id}
        ...
        <div className="p-2">
          <button
            onClick={addLine}
            className="text-orange-400 text-xs f-mono flex items-center gap-1 hover:text-orange-300"
          >
