...
import {
  ...
  RotateCcw, History, ShoppingCart, MessageSquare,
} from "lucide-react";
...
export default function JobDetailPage() {
  ...
  const fetchJob = async () => {
    ...
    const [{ data: jobData, error: jobErr }, { data: poData, error: poErr }] = await Promise.all([
      supabase
        .from("jobs")
        .select("*, job_line_items(*, parts(part_no, sku, description))")
        .eq("id", jobId)
        .eq("org_id", orgId)
        .single(),
      supabase
            ...
            {(job.job_line_items || []).length === 0 ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full min-w-[600px]">
                  <thead>
                    <tr><Th>Product</Th><Th>Code/SKU</Th><Th className="text-right">Qty</Th><Th className="text-right">Part Cost</Th></tr>
                  </thead>
                  <tbody>
        ...
        {tab === "issued" && (
          <Panel title="Issued Materials" icon={FileText}>
            <div className="text-sm text-slate-500 p-2">
              Not built yet — this will track materials formally issued to this job
              (separate from what's planned), in a future session.
            </div>
          </Panel>
        )}
