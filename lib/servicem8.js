...
// expected: firing 5 concurrent requests (the previous approach) burst
// past it hard enough that even the automatic 429 backoff in sm8Fetch
// couldn't recover. We can afford to be conservative here now — the
// database side (job upserts + material processing) is a single bulk call
// each since 007_bulk_sync_functions.sql, so it's no longer the fetch
// pacing that determines whether a sync finishes inside the time limit.
//
// A single job's fetch failing (e.g. it still gets rate-limited after
// sm8Fetch's own retries) doesn't abort the whole sync — that job's
// materials are just skipped for this run rather than losing everything
// that already succeeded.
const MATERIALS_FETCH_PAUSE_MS = 350;

export async function fetchJobMaterialsForJobs(accessToken, jobUuids) {
  const results = [];
  for (let i = 0; i < jobUuids.length; i++) {
    try {
      const materials = await fetchJobMaterialsForJob(accessToken, jobUuids[i]);
      results.push(...materials);
    } catch (e) {
      console.log(`[sm8 sync] skipped materials for job ${jobUuids[i]} after fetch failure: ${e.message}`);
    }
    if (i < jobUuids.length - 1) await sleep(MATERIALS_FETCH_PAUSE_MS);
  }
  return results;
}
