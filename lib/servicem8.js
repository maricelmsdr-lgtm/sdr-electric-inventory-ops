...
//   - TIME_BUDGET_MS catches scattered failures too (e.g. every other job
//     failing) — each retried failure can cost up to ~6s even spread out,
//     and that adds up past the route's 60s limit regardless of whether
//     the failures are consecutive or not.
...
export async function fetchJobMaterialsForJobs(accessToken, jobUuids) {
  ...
  for (let i = 0; i < shuffled.length; i++) {
    if (Date.now() - loopStart > TIME_BUDGET_MS) {
      console.log(`[sm8 sync] materials fetch hit its time budget (${TIME_BUDGET_MS}ms) — stopping early, ${shuffled.length - i} job(s) left unfetched this run.`);
      break;
    }
    try {
