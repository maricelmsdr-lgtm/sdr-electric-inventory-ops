...
//
// A single job's fetch failing doesn't abort the whole sync — that job's
// materials are just skipped for this run. But if the rate limit is
// genuinely being hit hard (several jobs in a row still failing after
// sm8Fetch's own retries), keep trying every remaining job anyway would
// just burn the rest of the time budget on requests likely to fail the
// same way — so after several consecutive failures, stop fetching more
// and proceed with whatever was gathered. A later sync picks up whatever
// got skipped (nothing here is lost, just deferred).
const MATERIALS_FETCH_PAUSE_MS = 600;
const MAX_CONSECUTIVE_FAILURES = 3;

export async function fetchJobMaterialsForJobs(accessToken, jobUuids) {
  // Randomize order so that if the circuit breaker below trips partway
  // through, a later sync attempt is trying different jobs rather than
  // hitting the exact same early ones and cutting off at the exact same
  // point every time — this way coverage converges across a few syncs
  // even during a sustained rate-limit issue.
